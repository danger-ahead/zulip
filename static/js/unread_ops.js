    ...
    channel.post({
        ...
        success(data) {
            ...
            if (!data.found_newest) {
                ...
                if (loading_indicator_displayed) {
                    // If we'd previously displayed a loading indicator,
                    ui_report.loading(
                        $t_html(
                            {defaultMessage: "Done! {N} messages marked as read."},
                            {N: args.unread_count_before_marking_all_msgs_read},
                        ),
                        $("#request-progress-status-banner"),
                        true,
                    );
        ...
        error(xhr) {
            ...
            if (xhr.responseJSON.code === "RATE_LIMIT_HIT") {
                setTimeout(() => mark_all_as_read(args), xhr.responseJSON["retry-after"]);
            } else {
